...
The Anthropic API key lives **only here**, so it's never exposed to the browser — which is the whole reason the static site grew a server.
...
widget that strangers will hit, you'll probably want to set cheaper models in
`.env`:

```env
CHAT_MODEL=claude-haiku-4-5      # plenty for portfolio Q&A, ~5x cheaper
RESUME_MODEL=claude-sonnet-4-6   # stronger reasoning for the fit analysis
```
...
1. Deploy this `server/` folder; set `ANTHROPIC_API_KEY` (and optionally
